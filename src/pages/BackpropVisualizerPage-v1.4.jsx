...
// ─── Design tokens — mirrors existing project CSS variables exactly ───────────
const C = {
  bg:      '#f7f6f3',
  bg2:     '#ffffff',
  bg3:     '#f0eef9',
...
// ─── Training samples: 10 × 12-pixel (4 rows × 3 cols) patterns ──────────────
const SAMPLES = [
...
function Label({ children, style = {} }) {
  return (
    <div style={{
      ...
      letterSpacing: '1.5px', marginBottom: 10, ...style,
    }}>
      {children}
    </div>
  )
...
function Tag({ children, color = C.accent, bg = C.tagBg }) {
  return (
    <span style={{
      ...
      padding: '2px 10px', borderRadius: 99, letterSpacing: 0.5,
    }}>
      {children}
    </span>
  )
...
// 4×3 pixel grid display
function PixelGrid({ pixels, cellPx = 18, highlightOn = false }) {
  return (
...
// Single neuron circle
function Neuron({ value, label, size = 46, glow = false, sub }) {
  const alpha = Math.min(0.12 + Math.abs(value ?? 0) * 0.6, 0.88)
  return (
      ...
      {STEP_META.map((s, i) => (
        <React.Fragment key={i}>
          ...
          {i < STEP_META.length - 1 && (
            <div style={{
              flexShrink: 0, width: 14, height: 1.5, margin: '0 2px',
              background: step > i ? C.accent : C.border, transition: 'background .3s',
            }} />
          )}
        </React.Fragment>
      ))}
...
function LossCurve({ history, epoch }) {
  ...
  useEffect(() => {
    ...
    ctx.fillText(`epoch ${epoch}`, W - 68, 12)
  }, [history, epoch])
  return (
    <canvas ref={ref} width={560} height={64}
...
export default function BackpropVisualizerPage() {
  ...
  function trainOneEpoch() {
    ...
    setWeights(w)
    setEpoch(e => e + 1)
    setLossHistory(h => [...h.slice(-299), r4(loss / SAMPLES.length)])
    setStep(0)
  }
  ...
  async function trainN(n) {
    ...
    while (autoRef.current && ep < n) {
      ...
      for (const s of SAMPLES) {
        const f = forward(s.pixels, w)
        const g = computeGrads(s.pixels, f.a2, f.a3, s.target, w.w2)
        w = applyUpdate(w, s.pixels, f.a2, g, lr); loss += g.loss
      }
      ep++
      if (ep % 20 === 0) {
  ...
  const dispLoss   = grads?.loss
    ?? (fwd ? r4(0.5 * fwd.a3.reduce((s, o, k) => s + (target[k] - o) ** 2, 0)) : null)
  const formula    = FORMULAS[Math.min(step, FORMULAS.length - 1)]

  const accuracy = epoch === 0 ? null : (() => {
    const n = SAMPLES.filter(s => {
  ...
  return (
    <div>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          Main layout: uses CSS grid, collapses on narrow viewports
      ══════════════════════════════════════════════════════════════════════ */}
      <style>{`
      ...
      <div className='bp-grid' style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
          ...
          <Card style={{ padding: 20 }}>
            <Label>NEURAL NETWORK  12 → 3 → 2</Label>

            {/* Horizontally scrollable if screen is very narrow */}
            <div style={{ overflowX: 'auto', paddingBottom: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 560 }}>

                {/* INPUT */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: C.text3, letterSpacing: 1 }}>INPUT</div>
                  <PixelGrid pixels={inputs} cellPx={22} highlightOn={step >= 1} />
                  {/* compact 12-node strip */}
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, marginTop: 2 }}>
                    {inputs.map((v, i) => (
                      <div key={i} style={{
                        ...
                        fontSize: 8, color: v === 1 ? C.accent : C.text3,
                        fontWeight: 700, fontFamily: "'DM Mono',monospace", transition: 'all .3s',
                      }}>{v}</div>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                  ...
                  {[0, 1, 2].map(i => (
                    <Neuron
                      ...
                      glow={step === 1 || step === 4}
                      sub={
                        step === 4 && grads ? `δ=${r3(grads.d2[i])}` :
                        step >= 1 && fwd    ? `z=${r3(fwd.z2[i])}`   : undefined
                      }
                    />
                  ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                  ...
                  {[0, 1].map(k => {
                    ...
                    const col = step >= 2 ? (isTarget ? C.green : C.danger) : C.accent
                    return (
                      <div key={k} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                        ...
                        <div style={{ textAlign: 'center', lineHeight: 1.5 }}>
                          ...
                          {step >= 4 && grads && (
                            <div style={{ fontSize: 8, color: C.orange, fontFamily: "'DM Mono',monospace" }}>
                              δ={r3(grads.d3[k])}
                            </div>
                          )}
                        </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, marginLeft: 8, flexShrink: 0 }}>
                  ...
                  <div style={{
                    ...
                    boxShadow: step >= 2 ? `0 0 18px ${isCorrect ? C.green : C.danger}33` : 'none',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                    transition: 'all .4s',
                  }}>
                    <span style={{ fontSize: 22, fontWeight: 800, color: step >= 2 ? (isCorrect ? C.green : C.danger) : C.text3 }}>
                      {step >= 2 ? pred : '?'}
                    </span>
          ...
          {/* Weight tables — side by side on wide, stacked on narrow */}
          <style>{`
            @media (max-width: 700px) {
              .bp-wtables { grid-template-columns: 1fr !important; }
            }
          `}</style>
          <div className='bp-wtables' style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    ...
                    {weights.w1.map((row, i) => (
                      <tr key={i}>
                        ...
                        {row.map((w, j) => {
                          ...
                          const isHov = hoveredW?.l === 'w1' && hoveredW.i === i && hoveredW.j === j
                          return (
                            <td key={j}
                              ...
                              style={{
                                ...
                                background: isHov ? C.tagBg : wBg(w),
                                color: wFg(w), fontWeight: 600,
                                outline: step === 5 && delta ? `1px solid ${C.orange}` : 'none',
                                transition: 'background .15s', cursor: 'default', minWidth: 28,
                              }}>
                              {w.toFixed(2)}
                              {step === 5 && delta !== null && (
                                <div style={{ fontSize: 7, color: C.orange }}>{delta > 0 ? '+' : ''}{delta.toFixed(3)}</div>
                              )}
                            </td>
                          )
                        })}
            ...
            <Card style={{ padding: 16 }}>
                  ...
                  {weights.w2.map((row, k) => (
                    <tr key={k}>
                      ...
                      {row.map((w, i) => {
                        ...
                        const isHov = hoveredW?.l === 'w2' && hoveredW.k === k && hoveredW.i === i
                        return (
                          <td key={i}
                            ...
                            {w.toFixed(3)}
                            {step === 5 && delta !== null && (
                              <div style={{ fontSize: 9, color: C.orange }}>{delta > 0 ? '+' : ''}{delta.toFixed(4)}</div>
                            )}
                          </td>
                        )
                      })}
              ...
              {/* δ display during backprop */}
              {step >= 4 && grads && (
                <div style={{ marginTop: 10, padding: '8px 10px', background: C.bg3, borderRadius: 8 }}>
                  <div style={{ fontSize: 9, color: C.orange, fontWeight: 700, letterSpacing: 1, marginBottom: 5 }}>
                    GRADIENTS
                  </div>
              ...
              {hoveredW && (
                <div style={{ marginTop: 8, padding: '5px 10px', background: C.tagBg, borderRadius: 6, fontSize: 10, color: C.text }}>
                  ...
                  {hoveredW.delta !== null && (
                    <span style={{ color: C.orange }}> → Δ = {hoveredW.delta.toFixed(4)}</span>
                  )}
                </div>
              )}
            </Card>
          ...
          <style>{`
            @media (max-width: 640px) {
              .bp-concepts { grid-template-columns: 1fr !important; }
            }
          `}</style>
          <div className='bp-concepts' style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {[
              ...
              <div key={title} style={{ background: C.bg2, borderRadius: 12, padding: '12px 14px', border: `1px solid ${C.border}` }}>
                ...
                <pre style={{ fontSize: 10, color: C.text2, lineHeight: 1.7, margin: 0, whiteSpace: 'pre-wrap', fontFamily: "'DM Mono',monospace" }}>
                  {body}
                </pre>
              </div>
            ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Card style={{ padding: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 5, marginBottom: 12 }}>
              {SAMPLES.map((s, i) => (
                <button key={i}
                  onClick={() => { setSampleIdx(i); setStep(0) }}
                  style={{
                    border: `2px solid ${sampleIdx === i ? C.accent : C.border}`,
                    background: sampleIdx === i ? C.tagBg : C.bg,
                    borderRadius: 10, padding: '5px 3px', cursor: 'pointer',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
                    transition: 'all .2s',
                  }}>
                  <PixelGrid pixels={s.pixels} cellPx={13} highlightOn={sampleIdx === i} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div style={{ background: C.bg3, borderRadius: 10, padding: '10px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: C.accent, fontFamily: "'DM Mono',monospace" }}>
                  [{target.join(', ')}]
                </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 13, fontWeight: 700, fontFamily: "'DM Mono',monospace", color: step >= 2 ? (isCorrect ? C.green : C.danger) : C.text3 }}>
                  {step >= 2 ? `[${out.map(v => r3(v)).join(', ')}]` : '[—, —]'}
                </div>
                <div style={{ fontSize: 10, fontWeight: 600, marginTop: 2, color: step >= 2 ? (isCorrect ? C.green : C.danger) : C.text3 }}>
          ...
          <Card style={{ padding: 16 }}>
            ...
            <Btn onClick={nextStep} disabled={autoRunning} variant='primary' style={{ marginBottom: 8, fontSize: 13 }}>
          ...
          <Card style={{ padding: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <Btn onClick={trainOneEpoch} disabled={autoRunning} variant='green'>
                ⚡ Train 1 Epoch (10 samples)
              </Btn>
              <Btn onClick={() => trainN(100)} disabled={autoRunning} variant='outline'>
                🚀 Train 100 Epochs
              </Btn>
              <Btn onClick={() => trainN(1000)} disabled={autoRunning} variant='orange'>
                ⚡ Train 1000 Epochs
              </Btn>
              {autoRunning
                ? <Btn onClick={() => { autoRef.current = false; setAutoRunning(false) }} variant='danger'>⬛ Stop</Btn>
                : <Btn onClick={resetAll} variant='ghost'>↺ Reset Weights</Btn>
              }
            </div>
            {autoRunning && (
          ...
          <Card style={{ padding: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: C.text3, marginBottom: 8 }}>
              <span>0.01</span>
              <span style={{ color: C.accent }}>0.2 (Excel 기본)</span>
              <span>1.0</span>
            </div>
            <div style={{
          ...
          {/* Formula */}
          <Card style={{ padding: 16 }}>
            <Label>FORMULA · {formula.label}</Label>
